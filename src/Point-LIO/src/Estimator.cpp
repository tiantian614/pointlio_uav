...
bool point_selected_surf[100000] = {0}; // hr: 满足条件的点面距离，用于LiDAR残差的更新
    ...
    for (int j = 0; j < time_seq[k]; j++) {
        ...
        if (point_selected_surf[idx + j + 1]) {
            ...
            if (extrinsic_est_en) {
                ...
                ekfom_data.h_x.block<1, 12>(m, 0) << norm_vec(0), norm_vec(1), norm_vec(2), VEC_FROM_ARRAY(
                        A), VEC_FROM_ARRAY(B), VEC_FROM_ARRAY(C);
            } else {
...
/* hr: 论文中提到的IMU每个通道的饱和（爆量程）检查，饱和度通道不会用于状态更新 */
void h_model_IMU_output(state_output &s, esekfom::dyn_share_modified<double> &ekfom_data) {
    ...
    ekfom_data.R_IMU
            << imu_meas_omg_cov, imu_meas_omg_cov, imu_meas_omg_cov, imu_meas_acc_cov, imu_meas_acc_cov, imu_meas_acc_cov;
    if (check_satu) {
...
void pointBodyToWorld(PointType const *const pi, PointType *const po) {
    ...
    if (extrinsic_est_en) {
        ...
        if (!use_imu_as_input) {
            p_global =
                    kf_output.x_.rot * (Lidar_R_wrt_IMU * p_body + Lidar_T_wrt_IMU) + kf_output.x_.pos; // .normalized()
        } else {
            p_global =
                    kf_input.x_.rot * (Lidar_R_wrt_IMU * p_body + Lidar_T_wrt_IMU) + kf_input.x_.pos; // .normalized()
        }
    }
