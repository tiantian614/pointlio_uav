...
void Preprocess::process_cut_frame_livox(const livox_ros_driver::CustomMsg::ConstPtr &msg,
    ...
    // 将普通处理后的合法点云分割成多帧 TODO 什么意思，分割数量是1啊，感觉没啥变化？？？ 目的应该是为了实现逐点进行更新
    // end time of last frame，单位ms
    double last_frame_end_time = msg->header.stamp.toSec() * 1000;
//    cout << "last_frame_end_time: " << last_frame_end_time << endl;
    uint valid_num = 0;
    uint cut_num = 0;
...
//        cout << "msg header stamp: " << msg->header.stamp.toSec() * 1000 << "   pl_surf[i].curvature: "
//             << pl_surf[i].curvature << "  last_frame_end_time: " << last_frame_end_time << endl;
        pcl_cut.push_back(pl_surf[i]);
        // hr 因为required_cut_num为1，因此valid_num == valid_pcl_size - 1时，即当前帧所有点都处理完时，进入if
