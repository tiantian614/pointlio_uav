...
std::string lid_topic, imu_topic, gps_topic;
...
/* hr multi lidar parameters */
bool use_multi_lidar;
queue<livox_ros_driver::CustomMsg::ConstPtr> livox1_queue, livox2_queue;
vector<double> extrinT_br(3, 0.0);
vector<double> extrinR_br(9, 0.0);
Eigen::Matrix4d tf_base2ref = Eigen::Matrix4d::Identity();
std::string lid_topic_multi;
...
void readParameters(ros::NodeHandle &nh) {
    ...
    nh.param<std::string>("common/gps_topic", gps_topic, "/mavros/local_position/pose");
    ...
    nh.param<bool>("publish/trajectory_save_en", trajectory_save_en, 1);
    ...
    /* hr multi lidar parameters */
    nh.param<int>("multi_lidar/lidar_type_multi", lidar_type_multi, 1);
    nh.param<bool>("multi_lidar/use_multi", use_multi_lidar, false);
    nh.param<string>("multi_lidar/lid_topic_multi", lid_topic_multi, "/livox/lidar");
    nh.param<int>("multi_lidar/scan_line_multi", p_pre->N_SCANS_MULTI, 6);
    nh.param<vector<double>>("multi_lidar/extrinsic_T_br", extrinT_br, vector<double>());
    nh.param<vector<double>>("multi_lidar/extrinsic_R_br", extrinR_br, vector<double>());
