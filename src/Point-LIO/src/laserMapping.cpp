...
void set_twist(T &out) {
        ...
        // ROS_INFO("%d", Measures.imu.size());
        // ROS_INFO("%lf", Measures.imu.back()->angular_velocity.x);

        ang_vel_meas(0) = imu_deque.back()->angular_velocity.x;
        ang_vel_meas(1) = imu_deque.back()->angular_velocity.y;
        ang_vel_meas(2) = imu_deque.back()->angular_velocity.z;
        out.angular.x = ang_vel_meas(0) - kf_output.x_.bg(0);
        out.angular.y = ang_vel_meas(1) - kf_output.x_.bg(1);
        out.angular.z = ang_vel_meas(2) - kf_output.x_.bg(2);
...
void publish_path(const ros::Publisher pubPath) {
 ...
 void timer_cbk(const ros::TimerEvent& time_e)
{
    //odompub_flag = true;
    if(odompub_flag)
...
void CallbackA(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    //ROS_INFO("I heard: [%f]", msg->pose.pose.position.z);
    //ROS_INFO("I heard: [%f]", msg->pose.pose.orientation.x);
    //ROS_INFO("I heard: [%f]", msg->pose.pose.orientation.y);
    //ROS_INFO("I heard: [%f]", msg->pose.pose.orientation.z);
    //ROS_INFO("I heard: [%f]", msg->pose.pose.orientation.w);
...
// void change_fre()
// {
//     //ros::Timer timer1 = nh.createTimer(ros::Duration(0.005), timer_cbk);
//     ros::spin();
// }

void gps_cbk(const geometry_msgs::PoseStamped::ConstPtr &gps_msg){
    //gps_msg->header.stamp = ros::Time().fromSec(lidar_end_time);
    string traj_file = root_dir + "/Log/gps_file.txt";
    ofstream ofs;
    ofs.open(traj_file, ios::out | std::ios::app);
    if (!ofs.is_open()) {
//            LOG(ERROR) << "Failed to open traj_file: " << traj_file;
        cerr << "Failed to open traj_file: " << traj_file;
    }
    // ofs << "#timestamp x y z q_x q_y q_z q_w" << std::endl;
    //for (auto &p: gps_msg.pose) {
        ofs << std::fixed << std::setprecision(6) << gps_msg->header.stamp.toSec() << " " << std::setprecision(15)
            << gps_msg->pose.position.x << " " << gps_msg->pose.position.y << " " << gps_msg->pose.position.z << " "
            << gps_msg->pose.orientation.x << " " << gps_msg->pose.orientation.y << " " << gps_msg->pose.orientation.z << " "
            << gps_msg->pose.orientation.w << std::endl;
    //}
    ofs.close();
}
...
int main(int argc, char **argv) {
    ...
    spinner.start();
    readParameters(nh);
    /* hr 读入多雷达外参矩阵： ref->base */
    if (use_multi_lidar) {
        Eigen::Vector3d T_base2ref = Eigen::Vector3d(0, 0, 0);
        Eigen::Matrix3d R_base2ref = Eigen::Matrix3d::Identity();
        T_base2ref << VEC_FROM_ARRAY(extrinT_br);
        R_base2ref << MAT_FROM_ARRAY(extrinR_br);
        cout << "R_base2ref Euler angle: " << endl << R_base2ref.eulerAngles(2, 1, 0).transpose() << endl;
        cout << "T_base2ref : " << endl << T_base2ref.transpose() << endl;
        tf_base2ref.block<3, 3>(0, 0) = R_base2ref.matrix();
        tf_base2ref.block<3, 1>(0, 3) = T_base2ref;
    }
    cout << "lidar_type: " << lidar_type << endl;
    if (use_multi_lidar)
        cout << "lidar_type_multi " << lidar_type_multi << endl;

    ivox_ = std::make_shared<IVoxType>(ivox_options_);


    path.header.stamp = ros::Time().fromSec(lidar_end_time);
    //stamp_gps = ros::Time().fromSec(lidar_end_time);
    ...
    p_imu->lidar_type = p_pre->lidar_type = lidar_type; // hr: no use
    p_imu->imu_en = imu_en;

    // hr: 修改版的init_dyn_share(get_f, df_dx, df_dw, h_share_model, NUM_MAX_ITERATIONS, epsi);
    // NUM_MAX_ITERATIONS默认为1
    ...
    reset_cov_output(P_init_output); // hr: 初始化状态量协方差矩阵 30 * 30
    kf_output.change_P(P_init_output);
    Eigen::Matrix<double, 24, 24> Q_input = process_noise_cov_input();
    Eigen::Matrix<double, 30, 30> Q_output = process_noise_cov_output(); // hr: 初始化噪声协方差矩阵 30 * 30
    /*** debug record ***/
    FILE *fp;
    ...
    /* hr: multi lidar merge */
#ifdef MultiLidar
   ros::Subscriber sub_livox_base = nh.subscribe(lid_topic, 200000, livox_base_callback);
   ros::Subscriber sub_livox_ref = nh.subscribe(lid_topic_multi, 200000, livox_ref_callback);
   std::thread sync_thread(sync_multi_avia);
#else
    ...
    //ros::Publisher pubOdomAftMapped = nh.advertise<nav_msgs::Odometry>("/Odometry", 1000);
    ...
    //ros::Subscriber sub_odom = nh.subscribe("/Odometry", 200000, odom_cbk);    
    //std::thread fre_thread(change_fre);

    ros::Timer timer1 = nh_2.createTimer(ros::Duration(0.005), timer_cbk);
    pubOdomAftMapped = nh_2.advertise<nav_msgs::Odometry>("/Odometry", 1000);
    ros::CallbackQueue callback_queue_a;
    nh_2.setCallbackQueue(&callback_queue_a);
    //ros::Subscriber sub_a = nh_2.subscribe("Odometry", 1000, CallbackA);
    std::thread spinner_thread_a([&callback_queue_a]() {
     ros::SingleThreadedSpinner spinner_a;
     spinner_a.spin(&callback_queue_a);
   });
    std::thread thread_debug(&debug);
        ...
        if (sync_packages(Measures)) {
            ...
            if (flg_first_scan) {
                ...
                if (imu_en) {
                    ...
                    {
                        // hr: 初始化时保证imu_next的时间戳大于等于第一帧LiDAR的时间戳
...
//            }
            ...
            /* main algorithm */
            if (!use_imu_as_input) {
                bool imu_upda_cov = false; // no use
                effct_feat_num = 0;
                        ...
                        if (!kf_output.update_iterated_dyn_share_modified()) {
                            ...
                            if (time_seq.size() > 0) {
                                ...
                                for (k = 0; k < time_seq.size(); k++) {
                                    ...
                                    if (imu_en && !imu_deque.empty()) {
                                        bool last_imu = imu_next.header.stamp.toSec() ==
                                                        imu_deque.front()->header.stamp.toSec();
 ...
 << " " << kf_output.x_.omg.transpose() << " " << kf_output.x_.acc.transpose() << " "
 ...
 << " " << kf_output.x_.omg.transpose() << " " << kf_output.x_.acc.transpose() << " "
                                         ...
                                         << " " << kf_output.x_.ba.transpose() << " " << feats_undistort->points.size()
                                         << endl;
                            }
                        }
 ...
 << " " << kf_input.x_.bg.transpose() << " " << kf_input.x_.ba.transpose() << " " << kf_input.x_.gravity.transpose()
 ...
 << " " << kf_output.x_.omg.transpose() << " " << kf_output.x_.acc.transpose() << " "
                                 << kf_output.x_.gravity.transpose() << " " << kf_output.x_.bg.transpose() << " "
                                 << kf_output.x_.ba.transpose() << " " << feats_undistort->points.size() << endl;
                    } else {
                        fout_out << setw(20) << Measures.lidar_beg_time - first_lidar_time << " "
                                 << euler_cur.transpose() << " " << kf_input.x_.pos.transpose() << " "
                                 << kf_input.x_.vel.transpose() \
 << " " << kf_input.x_.bg.transpose() << " " << kf_input.x_.ba.transpose() << " " << kf_input.x_.gravity.transpose()
    ...
    if (trajectory_save_en) {
        string traj_file = root_dir + "/Log/traj_file.txt";
        ofstream ofs;
        ofs.open(traj_file, ios::out);
        if (!ofs.is_open()) {
//            LOG(ERROR) << "Failed to open traj_file: " << traj_file;
            cerr << "Failed to open traj_file: " << traj_file;
        }
